...
export async function transformProjectPage(page: any, publicToPrivateMap?: Map<string, string>): Promise<Card> {
  ...
  if (daysLateValue !== null && daysLateValue !== undefined) {
    ...
    if (!isNaN(daysLateNum)) {
      ...
      // Store as null if invalid
      extractedProperties.days_late = null;
    }
  } else if (daysLateProp) {
    // Property exists but formula returned null - store as null
  ...
  if (lateValue !== null && lateValue !== undefined) {
    ...
    extractedProperties.late = null;
    extractedProperties.is_late = null;
  }
